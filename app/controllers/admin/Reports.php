...
class Reports extends MY_Controller
{
    ...
    function getPaymentsReport($pdf = NULL, $xls = NULL)
    {
        ...
        if ($pdf || $xls) {
            ...
            $this->load->library('datatables');
            $this->datatables
                ->select("DATE_FORMAT({$this->db->dbprefix('payments')}.date, '%Y-%m-%d %T') as date, " . $this->db->dbprefix('payments') . ".reference_no as payment_ref, " . $this->db->dbprefix('sales') . ".reference_no as sale_ref, " . $this->db->dbprefix('purchases') . ".reference_no as purchase_ref, paid_by, amount," . $this->db->dbprefix('payments') . ".type as type, {$this->db->dbprefix('payments')}.id as id")
                ->from('payments')
                ->join('sales', 'payments.sale_id=sales.id', 'left')
                ->join('purchases', 'payments.purchase_id=purchases.id', 'left')
                ->group_by('payments.id');
    ...
    function customer_report($user_id = NULL)
    {
        ...
        //cheques
        // $this->data['total_of_pending_cheques'] = $this->reports_model->getChequePurchasePayments_sales($user_id);
        $this->data['payments'] = $this->reports_model->getChequePurchasePayments_sales($user_id);
    ...
    function supplier_report($user_id = NULL)
    {
        ...
        //cheque payment
        $this->data['payments'] = $this->reports_model->getChequePurchasePayments($user_id);
        // $this->data['pending'] = count( $this->reports_model->getChequePurchasePayments($user_id));
