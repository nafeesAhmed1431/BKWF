...
class Products extends MY_Controller
{
    ...
    {
        // $products = $this->products_model->getAllProductsss();
        // //print_r($products[1]->id);exit;
        ...
        //         'avg_cost' => $p->cost
        //     );
        //    $a = $this->products_model->insertWareInWareHouse($data);
        // }
        // exit;
    ...
    function getProducts($warehouse_id = NULL)
    {
        ...
        $this->datatables->where('product_type', "default");
    ...
    function add($id = NULL)
    {
        ...
        if ($this->input->post('type') == 'standard' || $this->input->post('type') == 'production') {
        ...
        if ($this->form_validation->run() == true) {
            ...
            if ($this->input->post('type') == 'service') {
            ...
            } elseif ($this->input->post('type') == 'combo' || $this->input->post('type') == 'production') {
                $total_price = 0;
                ...
                if ($this->sma->formatDecimal($total_price) != $this->sma->formatDecimal($this->input->post('cost'))) {
        ...
        if ($this->form_validation->run() == true && $this->products_model->addProduct($data, $items, $warehouse_qty, $product_attributes, $photos)) {
            ...
            $this->data['combo_items'] = ($id && $this->data['product']->type == 'combo') ? $this->products_model->getProductComboItems($id) : $this->products_model->getProductComboItems($id);
            ...
            $count = $this->products_model->getProductCount();
            if ($count <= 500) {
    ...
    function edit($id = NULL)
    {
        ...
        if ($this->form_validation->run() == true && $this->products_model->updateProduct($id, $data, $items, $warehouse_qty, $product_attributes, $photos, $update_variants)) {
            ...
            $this->data['combo_items'] = $product->type == 'combo' ? $this->products_model->getProductComboItems($product->id) : $this->products_model->getProductComboItems($product->id);
            $this->data['product_options'] = $id ? $this->products_model->getProductOptionsWithWH($id) : NULL;
                ...
                foreach ($arrResult as $key => $value) {
                    ...
                    if ($catd = $this->products_model->getCategoryByCode($item['category_code'])) {
                        ...
                        if ($product = $this->products_model->getProductByCode($item['code'])) {
                            if ($product->type == 'standard' || $product->type == 'production') {
                                if ($item['variants']) {
    ...
    function production_quantity_adjustments($warehouse_id = NULL)
    {
        $this->sma->checkPermissions('adjustments');
        ...
        $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => admin_url('products'), 'page' => lang('products')), array('link' => '#', 'page' => lang('quantity_adjustments')));
        $meta = array('page_title' => lang('quantity_adjustments'), 'bc' => $bc);
        $this->page_construct('products/production_quantity_adjustments', $meta, $this->data);
    }
    ...
    function getadjustments_production($warehouse_id = NULL)
    {
        $this->sma->checkPermissions('adjustments');
    ...
    function add_adjustment_production($count_id = NULL)
    {
        $this->sma->checkPermissions('adjustments', true);
        $this->form_validation->set_rules('warehouse', lang("warehouse"), 'required');
        ...
        if ($this->form_validation->run() == true) {
            ...
            for ($r = 0; $r < $i; $r++) {
                ...
                $comboitems[] = $this->products_model->getProductionItems($product_id);
            }
            $status = true;
            $i = 0;
            for ($j = 0; $j < count($comboitems); $j++) {
                foreach ($comboitems[$j] as $c) {
                    $comboitemsdata[$j][$i] = $this->products_model->getProductionItems_data($c->item_code);
                    $i++;
                }
                $i = 0;
            }
            ...
            for ($a = 0; $a < count($products); $a++) {
                foreach ($comboitems[$a] as $c) {
                    if (($c->quantity * $products[$a]['quantity']) >  $comboitemsdata[$a][$t][0]->quantity) {
                        $status = false;
                        $this->session->set_flashdata('error', lang('production_item_less_error'));
                        admin_redirect('products/add_adjustment_production');
                    } else {
                        $t++;
                    }
                }
            ...
            if ($status == true) {
                $this->products_model->update_production_items($comboitems, $comboitemsdata, $products);
            }
        ...
        if ($this->form_validation->run() == true && $this->products_model->addAdjustment($data, $products)) {
            $this->session->set_userdata('remove_qals', 1);
            $this->session->set_flashdata('message', lang("quantity_adjusted"));
            admin_redirect('products/production_quantity_adjustments');
        } else {
            ...
            $this->data['warehouses'] = $this->site->getAllWarehouses();
            $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => admin_url('products'), 'page' => lang('products')), array('link' => '#', 'page' => lang('add_adjustment')));
            $meta = array('page_title' => lang('add_adjustment_production'), 'bc' => $bc);
            $this->page_construct('products/add_adjustment_production', $meta, $this->data);
        }
    }
    ...
    function modal_view($id = NULL)
    {
        ...
        if ($pr_details->type == 'combo' || $pr_details->type == 'production') {
    ...
    public function qa_suggestions_production()
    {
        $term = $this->input->get('term', true);
        ...
        $rows = $this->products_model->getQASuggestions_production($sr);
        if ($rows) {
            foreach ($rows as $row) {
                $row->qty = 1;
                $options = $this->products_model->getProductOptions($row->id);
    ...
    {
        $arr['data'] = $this->Commons_model->get_where('sma_supplier', [true => true]);
    ...
    {
        $this->form_validation->set_rules('supplier_name', 'Supplier Name', 'trim|required|alpha');
        $this->form_validation->set_rules('supplier_phone', 'Supplier Phone', 'trim|required|numeric');
        $this->form_validation->set_rules('supplier_address', 'Supplier Address', 'required');
        if ($this->form_validation->run()) {
            $arr['status'] = $this->Commons_model->insert([
                'name' => $this->input->post('supplier_name'),
                'phone' => $this->input->post('supplier_phone'),
                'address' => $this->input->post('supplier_address'),
                'description' => $this->input->post('supplier_description')
            ], 'sma_supplier');
            if ($arr['status']) {
                $arr['data'] = $this->Commons_model->get_where('sma_supplier', [true => true]);
            }
    ...
    {
        $arr['data'] = $this->Commons_model->get_where('sma_categories', [true => true]);
        $arr['status'] = !empty($arr['data']);
        echo json_encode($arr);
    }
    ...
    function add_category()
    {
        ...
        $this->form_validation->set_rules('category_name', 'Category Name', 'trim|required|alpha');
        $this->form_validation->set_rules('category_slug', 'Category Slug', 'trim|required|alpha');
        $this->form_validation->set_rules('category_desc', 'Category Desc', 'trim|required');
        if ($this->form_validation->run()) {
            $id = $this->Commons_model->insert_with_id([
                'code' => $this->input->post('category_code'),
                'name' => $this->input->post('category_name'),
                'slug' => strtolower($this->input->post('category_slug')),
    ...
    function get_units()
    {
        $arr['data'] = $this->Commons_model->get_where('sma_units', [true => true]);
        $arr['status'] = !empty($arr['data']);
        echo json_encode($arr);
    }
