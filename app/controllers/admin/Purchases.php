...
class Purchases extends MY_Controller
{
    ...
    public function getPurchases($warehouse_id = null)
    {
        ...
        $cheque_payments_link = anchor('admin/purchases/cheque_payments/$1', '<i class="fa fa-money"></i> ' . 'View Cheque Payments', 'data-toggle="modal" data-target="#myModal"');
        ...
        if ($this->form_validation->run() == true && $this->purchases_model->addPurchase($data, $products)) {
            ...
            if ($quote_id) {
                ...
                foreach ($items as $item) {
                    ...
                    } elseif ($row->type == 'standard' || $row->type == 'production') {
                        if (!$row) {
                            $row = json_decode('{}');
                            $row->quantity = 0;
                        } else {
    ...
    public function edit($id = null)
    {
        ...
        if ($inv->type == "opening_balance") {
            $this->session->set_flashdata('error', lang('opening_balance_edit'));
            admin_redirect("purchases");
        }
    ...
    public function delete($id = null)
    {
        ...
        $inv = $this->purchases_model->getPurchaseByID($id);
        if ($inv->type == "opening_balance") {
            $this->session->set_flashdata('error', lang('opening_balance_delete'));
            admin_redirect("purchases");
        }
    ...
    public function cheque_payments($id = null)
    {
        $this->sma->checkPermissions(false, true);

        $this->data['payments'] = $this->purchases_model->getChequePurchasePayments($id);
        $this->data['inv'] = $this->purchases_model->getPurchaseByID($id);
        $this->load->view($this->theme . 'purchases/payments', $this->data);
    }
    ...
    public function add_payment($id = null)
    {
        ...
        if ($purchase->payment_status == 'paid' && $purchase->grand_total == $purchase->paid) {
            if ($purchase->opening_balance_type == null || $purchase->opening_balance_type == "") {
        ...
        if ($this->form_validation->run() == true) {
            ...
            $data = array(
                'date' => $date,
                'category_id' => $this->input->post('paid_by') . ' (Purchase)',
                'reference' => $this->input->post('reference_no') ? $this->input->post('reference_no') : $this->site->getReference('ppay'),
                'amount' => $this->input->post('amount-paid'),
                'created_by' => $this->session->userdata('user_id'),
                'note' => $this->sma->clear_tags($this->input->post('note')),
                'type' => 2,
        ...
        $cash_head = $this->purchases_model->get_cash_head();
        if ($this->form_validation->run() == true && $this->purchases_model->addPaymentNew($payment, $data)) :
            if ($cash_head) :
                $this->session->set_flashdata('message', lang("payment_added"));
                redirect($_SERVER["HTTP_REFERER"]);
            else :
                $this->session->set_flashdata('message', "Open Balance First");
                admin_redirect('purchases/cash_management');
            endif;
        else :
            $this->data['error'] = (validation_errors() ? validation_errors() : $this->session->flashdata('error'));
    ...
    public function edit_cheque_payment($id = null)
    {
        $this->sma->checkPermissions('edit', true);
        $this->load->helper('security');
        if ($this->input->get('id')) {
        ...
        } elseif ($this->input->post('edit_cheque_payment')) {
            $this->session->set_flashdata('error', validation_errors());
            redirect($_SERVER["HTTP_REFERER"]);
        }

        if ($this->form_validation->run() == true && $this->purchases_model->updateChequePayment($id, $payment)) {
            $this->session->set_flashdata('message', lang("payment_updated"));
            admin_redirect("purchases");
        } else {
            ...
            $this->data['payment'] = $this->purchases_model->getChequePaymentByID($id);
            $this->data['details'] = $this->purchases_model->getPurchaseByID($id);
            $this->data['modal_js'] = $this->site->modal_js();

            $this->load->view($this->theme . 'purchases/edit_cheque_payment', $this->data);
        }
    }

    public function edit_cheque_payment_reports($id = null)
    {
        $this->sma->checkPermissions('edit', true);
        $this->load->helper('security');
        if ($this->input->get('id')) {
        ...
        if ($this->form_validation->run() == true && $this->purchases_model->updateChequePayment($id, $payment)) {
            ...
            $this->load->view($this->theme . 'purchases/edit_cheque_payment_report', $this->data);
        }
    }
    ...
    public function delete_cheque_payment($id = null)
    {
        $this->sma->checkPermissions('delete', true);
    ...
    //cash changes
    public function cash_management($id = null)
    {
        $this->form_validation->set_rules('cash_in_hand', lang("cash_in_hand"), 'trim|required|numeric');
        $head = $this->purchases_model->get_cash_head();
        if ($head) :
            $this->sma->checkPermissions();

            $this->data['error'] = (validation_errors()) ? validation_errors() : $this->session->flashdata('error');
            $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => '#', 'page' => lang('Cash Management')));
            ...
            if ($this->form_validation->run() == TRUE && $this->purchases_model->insert_cash_head($data) && $this->purchases_model->insert_cash_head_management($management_data)) :
                $this->session->set_flashdata('message', "welcome to Cash Management System");
                admin_redirect("purchases/cash_management");
            else :
                $this->sma->checkPermissions();
                $this->data['error'] = (validation_errors()) ? validation_errors() : $this->session->flashdata('error');
                $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => '#', 'page' => 'Open Balance'));
                $meta = array('page_title' => 'Open Balance', 'bc' => $bc);
                $this->page_construct('purchases/open_balance', $meta, $this->data);
            endif;
        endif;
    }
    ...
    public function getCash()
    {
        ...
        $this->datatables
            ->select($this->db->dbprefix('cash_management') . ".id as id,date, reference,type, category_id, amount, note, CONCAT({$this->db->dbprefix('users')}.first_name, ' ', {$this->db->dbprefix('users')}.last_name) as user, attachment", false)
            ->from('cash_management')
            ->join('users', 'users.id=cash_management.created_by', 'left')
            ->join('cash_categories', 'cash_categories.id=cash_management.category_id', 'left')
            ->group_by('cash_management.id');
    ...
    public function add_expense()
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            if ($_FILES['userfile']['size'] > 0) {
                ...
                $photo = $this->upload->file_name;
                $data['attachment'] = $photo;
                $cash_head_data['attachment'] = $photo;
            }
        ...
        if ($this->form_validation->run() == true && $this->purchases_model->addExpense($data, $cash_head_data)) {
    ...
    public function add_cash()
    {
        ...
        if ($this->form_validation->run() == true && $this->purchases_model->addCash($data) && $this->purchases_model->update_cash_head($cash_head_data)) {
            $this->session->set_flashdata('message', lang("Cash successfully updates"));
            redirect($_SERVER["HTTP_REFERER"]);
        } else {
            $this->data['error'] = (validation_errors() ? validation_errors() : $this->session->flashdata('error'));
            $this->data['exnumber'] = ''; //$this->site->getReference('ex');
            $this->data['warehouses'] = $this->site->getAllWarehouses();
            $this->data['categories'] = $this->purchases_model->getCashCategories();
            $this->data['modal_js'] = $this->site->modal_js();
            $this->load->view($this->theme . 'purchases/add_cash', $this->data);
        }
    }
    ...
    public function return_purchase($id = null)
    {
        ...
        $purchase = $this->purchases_model->getPurchaseByID($id);
        if ($purchase->type == "opening_balance") {
            $this->session->set_flashdata('error', lang('opening_balance_return'));
            admin_redirect("purchases");
        }
    ...
    public function make_cheque_payment($id)
    {
        $cash_head_data = $this->input->post();
        if ($id) : $this->purchases_model->delete_cheque_payment_record($id, ['status' => 1], $cash_head_data);
        endif;
        admin_redirect('purchases');
    }
    public function make_cheque_payment_report($id)
    {
        $cash_head_data = $this->input->post();
        // print_r($cash_head_data);die;
        if ($id) : $this->purchases_model->delete_cheque_payment_record($id, ['status' => 1], $cash_head_data);
