...
class Sales extends MY_Controller
{
    ...
    public function getSales($warehouse_id = null)
    {
        ...
        $cheque_payments_link = anchor('admin/sales/cheque_payments/$1', '<i class="fa fa-money"></i> ' . 'View Cheque Payments', 'data-toggle="modal" data-target="#myModal"');
    ...
    public function view($id = null)
    {
        ...
        $bal = $this->sales_model->getCustomerBalance($inv->customer_id);
    ...
    {
        $this->sma->checkPermissions(false, true);
        $this->data['payments'] = $this->sales_model->getChequeInvoicePayments($id);
        $this->data['inv'] = $this->sales_model->getInvoiceByID($id);
        $this->load->view($this->theme . 'sales/payments', $this->data);
    }
    ...
    public function add_payment($id = null)
    {
        ...
        if ($this->form_validation->run() == true && $this->sales_model->addPaymentNew($payment, $customer_id, $cash_head_data)) {
            $cash = $this->purchases_model->get_cash_head();
            if ($cash) :
    ...
    public function edit_cheque_payment($id = null)
    {
        ...
        $payment = $this->sales_model->getChequePaymentByID($id);
        if ($payment->paid_by == 'ppp' || $payment->paid_by == 'stripe' || $payment->paid_by == 'paypal' || $payment->paid_by == 'skrill') {
            $this->session->set_flashdata('error', lang('x_edit_payment'));
            $this->sma->md();
        }
        ...
        if ($this->form_validation->run() == true && $this->sales_model->updatePayment($id, $payment, $customer_id)) {
            ...
            $this->data['modal_js'] = $this->site->modal_js();
            // print_r($payment);die;
            $this->load->view($this->theme . 'sales/edit_cheque_payment', $this->data);
        }
    }

    public function edit_cheque_payment_report($id = null)
    {
        $this->sma->checkPermissions('edit', true);
        $this->load->helper('security');
        if ($this->input->get('id')) {
        ...
        if ($this->form_validation->run() == true && $this->sales_model->updatePayment($id, $payment, $customer_id)) {
            ...
            $this->data['payment'] = $payment;
            $this->data['modal_js'] = $this->site->modal_js();
            $this->load->view($this->theme . 'sales/edit_cheque_payment_report', $this->data);
        }
    }
    ...
    public function receive_cheque_payment($id)
    {
        $cash_head_data = $this->input->post();
        if ($id) : $this->sales_model->delete_cheque_payment_record($id, $cash_head_data);
        endif;
        admin_redirect('sales');
    }


    public function receive_cheque_payment_report($id)
    {
        $cash_head_data = $this->input->post();
        // echo "<pre>";
