...
class Site extends CI_Model
{
    ...
    public function getPurchasePaymentsNew($purchase_id) {
    ...
    public function syncPurchasePaymentsNew($id, $amt) {
        // $purchase = $this->getPurchaseByID($id);
        // $q1 = $this->db->get_where('companies', array('id' => $purchase->supplier_id), 1);
        // $companies = $q1->row();
        // if($purchase->opening_balance_type == "payable" && $purchase->type == "opening_balance"){
        //     $data['type'] = "sent";
        //     if($amt == $purchase->total){
        //         $com['opening_balance'] = $purchase->total - $amt;
        //         $com['type'] = "payable";
        //         $p['opening_balance_type'] = 'receiveable';
        //     }else if($amt > $purchase->total){
        //         $balance = $purchase->total - $amt;
        //         $com['opening_balance'] = ($balance * -1);
        //         $com['type'] = "receiveable";
        //         $p['opening_balance_type'] = 'receiveable';
        //     }else{
        //         $com['opening_balance'] = $purchase->total - $amt;
        //         $com['type'] = "payable";
        //         $p['opening_balance_type'] = 'payable';
        //     }
        // $this->db->update('companies', $com, array('id' => $companies->id));
        // $this->db->update('purchases', $p, array('id' => $id));
        // }else if($purchase->opening_balance_type == "receiveable" && $purchase->type == "opening_balance"){
        //     $data['type'] = "received";
        //     if($amt == $purchase->total){
        //         $com['opening_balance'] = $purchase->total - $amt;
        //         $com['type'] = "receiveable";
        ...
        //     }
        // }else{
        //     if ($payments = $this->getPurchasePaymentsNew($id)) {
        //         foreach ($payments as $payment) {
        //             // $paid += $payment->amount;
        //             $paid = $payment->amount;
        //         }
        ...
        //     if ($this->db->update('purchases', array('paid' => ($purchase->paid + $paid), 'payment_status' => $payment_status), array('id' => $id))) {
        //         return true;
        //     }
        // }
    ...
    public function item_costing($item, $pi = NULL) {
        ...
        if ($this->Settings->accounting_method != 2 && !$this->Settings->overselling) {
            ...
            if ($this->getProductByID($item['product_id'])) {
                if ($item['product_type'] == 'standard' || $item['product_type'] == 'production') {
                    ...
                    foreach ($combo_items as $combo_item) {
                        ...
                        if ($pr->type == 'standard' || $pr->type == 'production') {
    ...
    public function costing($items) {
        ...
        foreach ($items as $item) {
            ...
            if ($pr && ($pr->type == 'standard' || $pr->type == 'production')) {
                ...
                foreach ($combo_items as $combo_item) {
                    if ($combo_item->type == 'standard' || $combo_item->type == 'production') {
