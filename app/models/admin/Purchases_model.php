...
class Purchases_model extends CI_Model
{
    ...
    {
        $this->db->where("(type = 'standard' OR type = 'production') AND (name LIKE '%" . $term . "%' OR code LIKE '%" . $term . "%' OR supplier1_part_no LIKE '%" . $term . "%' OR supplier2_part_no LIKE '%" . $term . "%' OR supplier3_part_no LIKE '%" . $term . "%' OR supplier4_part_no LIKE '%" . $term . "%' OR supplier5_part_no LIKE '%" . $term . "%' OR  concat(name, ' (', code, ')') LIKE '%" . $term . "%')");
    ...
    public function getChequePurchasePayments($purchase_id)
    {
        $this->db->order_by('id', 'asc');
        $q = $this->db->get_where('cheques_payments', ['purchase_id' => $purchase_id, 'status' => 0]);
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
                $data[] = $row;
            }
    ...
    public function getChequePaymentByID($id)
    {
        $q = $this->db->get_where('cheques_payments', array('id' => $id), 1);
        if ($q->num_rows() > 0) {
            return $q->row();
        }
    ...
    public function addPaymentNew($data = array(), $newdata)
    {
        if ($data['paid_by'] == 'Cheque') :
            if ($this->db->insert('cheques_payments', $data)) :
                if ($this->site->getReference('ppay') == $data['reference_no']) :
                    $this->site->updateReference('ppay');
                endif;
                $this->site->syncPurchasePaymentsNew($data['purchase_id'], $data['amount']);
            endif;
        else :
            if ($this->db->insert('payments', $data) && $this->addCash($newdata) && $this->update_cash_head($newdata)) :
                if ($this->site->getReference('ppay') == $data['reference_no']) :
                    $this->site->updateReference('ppay');
                endif;
                $this->site->syncPurchasePaymentsNew($data['purchase_id'], $data['amount']);
    ...
    public function updateChequePayment($id, $data = array())
    {
        if ($this->db->update('cheques_payments', $data, array('id' => $id))) {
            $this->site->syncPurchasePayments($data['purchase_id']);
            // $this->update_cash_head($update_head_data);
            return true;
        }
        return false;
    }
    ...
    {
        $opay = $this->getChequePaymentByID($id);
        if ($this->db->delete('cheques_payments', array('id' => $id))) {
            $this->site->syncChequePurchasePayments($opay->purchase_id);
            return true;
        }
        return FALSE;
    }
    ...
    public function addExpense($data = array(), $cash_head_data)
    {
        if ($this->db->insert('expenses', $data)) {
            if ($this->site->getReference('ex') == $data['reference']) {
                $this->site->updateReference('ex');
                $this->addCash($cash_head_data);
                $this->update_cash_head($cash_head_data);
            }
            return true;
        }
        return false;
    ...
    public function addCash($data = array())
    {
        if ($data["type"] == "1") {
            $data['type'] = "cashin";
        } else {
        ...
        if ($this->db->insert('cash_management', $data)) {
            if ($this->site->getReference('ex') == $data['reference']) {
                $this->site->updateReference('ex');
            }
            return true;
    ...
    public function insert_cash_head($data)
    {
        $res = $this->db->insert('sma_cash_head', $data);
        return $res ? $this->db->insert_id() : false;
    }

    public function insert_cash_head_management($management_data)
    {
        $res = $this->db->insert('sma_cash_management', $management_data);
        return $res ? $this->db->insert_id() : false;
    }

    public function get_cash_head()
    {
        $data = $this->db->query("SELECT * FROM `sma_cash_head`");
        return $data ? $data->row() : FALSE;
    }

    public function update_cash_head($cash_head_data)
    {
        $amount = $cash_head_data['amount'];
        $type = $cash_head_data['type'];
        $cash_head = $this->db->where(['start_bit' => 1])->get('cash_head')->row();
        if ($cash_head) {
            $cash_inhand = $cash_head->cash_inhand;
            $total_cash_in = $cash_head->total_cash_in;
            $total_cash_out = $cash_head->total_cash_out;
            $id = $cash_head->id;
            if ($type == 1) {
                $res = $this->cashin_cash_head($amount, $cash_inhand, $total_cash_in, $id);
                return $res ? true : false;
            } elseif ($type == 2) {
                $res = $this->cashout_cash_head($amount, $cash_inhand, $total_cash_out, $id);
                return $res ? true : false;
            }
        }
    }

    public function cashin_cash_head($amount, $cash_inhand, $total_cash_in, $id)
    {
        $new_cash_inhand = $amount + $cash_inhand;
        $new_cash_in = $amount + $total_cash_in;
        $data = array(
            'cash_inhand' => $new_cash_inhand,
            'total_cash_in' => $new_cash_in,
        );

        $this->db->where(['id' => $id]);
        $res = $this->db->update('cash_head', $data);

        return $res ? true : false;
    }

    public function cashout_cash_head($amount, $cash_inhand, $total_cash_out, $id)
    {
        $new_cash_inhand = $cash_inhand - $amount;
        $new_cash_out = $amount + $total_cash_out;
    ...
    public function getCashCategories()
    {
        $q = $this->db->get('cash_categories');
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
                $data[] = $row;
            }
    ...
    public function delete_cheque_payment_record($id, $arr, $cash_head_data)
    {
        unset($cash_head_data["make_cheque_payment"]);
        $res = $this->db->where(['id' => $id])->update('sma_cheques_payments', $arr);
        if ($res) :
            $cheque_payment_record = $this->db->select('*')->where(['status' => 1, 'id' => $id])->get('sma_cheques_payments')->row();
            $payment = json_decode(json_encode($cheque_payment_record), true);
            unset($payment['id'], $payment['date'], $payment['status']);
            $payment['date'] = date('Y-m-d h:i:s');
            $this->db->insert('sma_payments', $payment);
            $res = $this->addCash($cash_head_data);
            $this->update_cash_head($cash_head_data);
            $this->site->syncPurchasePaymentsNew($payment['purchase_id'], $payment['amount']);
