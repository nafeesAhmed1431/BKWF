...
class Reports_model extends CI_Model
{
    ...
    {
        $this->db->select('SUM(COALESCE(grand_total, 0)) as total_amount, SUM(COALESCE(paid, 0)) as paid', FALSE)->select('id')
    ...
    public function getChequePurchasePayments($user_id)
    {
        $purchase_id = 0;
        $this->db->select('id')
            ->where('supplier_id', $user_id);
        $s_id = $this->db->get('purchases')->result();
    ...
    // public function getPendingchequesSuppliers($user_id)
    // {
    //     $purchase_id = 0;
    //     $this->db->select('id')
    //         ->where('supplier_id', $user_id);
    ...
    //         $purchase_id = $sid->id;
    //     }
    //     $res = $this->db->get_where('cheques_payments', ['status' => 0, 'purchase_id' => $purchase_id])->num_rows();
    //     return $res;
    // }

    // public function getCustomerPendingCheques($user_id)
    // {
    //     $this->db->select('id')
    //         ->where('customer_id', $user_id);
    //     $s_id = $this->db->get('sales')->result();

    //     foreach ($s_id as $sid) {
    //         $this->db->order_by('id', 'asc');
    //         $res = $this->db->get_where('cheques_payments', array('sale_id' => $sid->id, 'status' => 0));
    //         if($res->num_rows() > 0){
    ...
    public function getChequePurchasePayments_sales($user_id)
    {
        $this->db->select('id')
            ->where('customer_id', $user_id);
        $s_id = $this->db->get('sales')->result();
        ...
        foreach ($s_id as $sid) {
            $this->db->order_by('id', 'asc');
            $q = $this->db->get_where('cheques_payments', array('sale_id' => $sid->id, 'status' => 0));
            if ($q->num_rows() > 0) {
                foreach (($q->result()) as $row) {
                    $data[] = $row;
                }
