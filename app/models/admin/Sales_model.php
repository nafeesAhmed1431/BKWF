...
class Sales_model extends CI_Model
{
    ...
    {
        parent::__construct();
        $this->load->model('admin/Purchases_model');
    }
    ...
    public function getCustomerBalance($cus_id)
    {
        $this->db->select('(sum(grand_total) - sum(paid)) as balance');
        $q = $this->db->get_where('sma_sales', array('customer_id' => $cus_id));
        if ($q->num_rows() > 0) {
            return $q->result()[0]->balance;
        } else {
            return 0;
        }
    }
    ...
    public function getChequeInvoicePayments($sale_id)
    {
        $this->db->order_by('id', 'asc');
        $q = $this->db->get_where('cheques_payments', array('sale_id' => $sale_id, 'status' => 0));
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
                $data[] = $row;
            }
    ...
    public function addPaymentNew($data = array(), $id, $cash_head_data)
    {
        ...
        if ($data['paid_by'] == 'Cheque') :
            ...
            if ($this->db->insert('payments', $data) && $this->Purchases_model->addCash($cash_head_data) && $this->Purchases_model->update_cash_head($cash_head_data)) :
                if ($this->site->getReference('ppay') == $data['reference_no']) :
                    $this->site->updateReference('ppay');
                endif;
                // $res = $this->site->syncPurchasePaymentsNew($data['sale_id'], $data['amount']);
                $this->site->syncSalePaymentsNew($data['sale_id'], $data['amount']);
            endif;
        endif;
        return true;
    }
    ...
    public function delete_cheque_payment_record($id, $cash_head_data)
    {
        $arr = [
            'status' => 1,
        ];
        unset($cash_head_data["sales/receive_cheque_payment"]);
        ...
        if ($res) :
            ...
            $res = $this->db->insert('sma_payments', $payment);

            $this->Purchases_model->addCash($cash_head_data);
            // print_r($res);die;
            $this->Purchases_model->update_cash_head($cash_head_data);
            $this->site->syncSalePaymentsNew($payment['sale_id'], $payment['amount']);
