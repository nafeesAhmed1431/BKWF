...
class Products_model extends CI_Model
{
    ...
    {
        $q = $this->db->get_where('products', array('product_type' => "default"));
        if ($q->num_rows() > 0) {
    ...
    // public function getAllProductsss()
    // {
    ...
    // public function insertWareInWareHouse($data)
    // {
    //     if ($this->db->insert('sma_warehouses_products', $data)) {
    ...
    {
        $q = $this->db->get_where('products', array('subcategory_id' => $subcategory_id, 'product_type' => "default"));
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
                $data[] = $row;
            }
    ...
    {
        $this->db->select($this->db->dbprefix('products') . '.id as id, ' . $this->db->dbprefix('products') . '.code as code, ' . $this->db->dbprefix('combo_items') . '.quantity as qty, ' . $this->db->dbprefix('products') . '.name as name, ' . $this->db->dbprefix('combo_items') . '.unit_price as price')->join('products', 'products.code=combo_items.item_code')->group_by('combo_items.id');
    ...
    public function getProductCount()
    {
        ...
        $q = $this->db->get_where($this->db->dbprefix('products'), array('product_type' => "type"));
    ...
    public function addProduct($data, $items, $warehouse_qty, $product_attributes, $photos){
        if ($this->db->insert('products', $data)) {
            ...
            if ($data['type'] != 'standard' && $data['type'] != 'production') {
    ...
    public function getProductNames($term, $limit = 10)
    {
        $this->db->select('' . $this->db->dbprefix('products') . '.id, code, ' . $this->db->dbprefix('products') . '.name as name, ' . $this->db->dbprefix('products') . '.price as price, ' . $this->db->dbprefix('product_variants') . '.name as vname')
            ->where("product_type = 'default' AND type != 'combo' AND type != 'production' AND "
    ...
    public function getQASuggestions($term, $limit = 5)
    {
        $this->db->select('' . $this->db->dbprefix('products') . '.id, code, ' . $this->db->dbprefix('products') . '.name as name')
            ->where("product_type = 'default' AND type != 'combo' AND "
                . "(" . $this->db->dbprefix('products') . ".name LIKE '%" . $term . "%' OR code LIKE '%" . $term . "%' OR
                concat(" . $this->db->dbprefix('products') . ".name, ' (', code, ')') LIKE '%" . $term . "%')")
            ->limit($limit);
        $q = $this->db->get('products');
    ...
    {
        //$q = $this->db->get_where('sma_combo_items', array('product_id' => $id), 1);
        $this->db->select('*');
        $this->db->from('sma_combo_items');
    ...
    public function update_production_items($comboitems, $comboitemsdata, $products)
    {
        //echo count($products);exit;
        $t = 0;
        for($a = 0; $a < count($comboitems); $a++){
            foreach($comboitems[$a] as $c){
                // $data = array(
                //     'quantity' => $comboitemsdata[$a][$t][0]->quantity - ($c->quantity * $products[$a]['quantity'])
                // );
                // $this->db->trans_start();
                $this->db->set('quantity', (float)$comboitemsdata[$a][$t][0]->quantity - (float)($c->quantity * $products[$a]['quantity']));
                $this->db->where('product_id',  $comboitemsdata[$a][$t][0]->id);
                $this->db->update('sma_warehouses_products');
                // $a = $this->db->update('sma_warehouses_products', $data, array('product_id' => $comboitemsdata[$a][$t][0]->id));
                $this->db->set('quantity', (float)$comboitemsdata[$a][$t][0]->quantity - (float)($c->quantity * $products[$a]['quantity']));
                $this->db->where('id',  $comboitemsdata[$a][$t][0]->id);
                $this->db->update('sma_products');
                // $b = $this->db->update('sma_products', $data, array('id' => $comboitemsdata[$a][$t][0]->id));
    ...
    public function getStockCountProducts($warehouse_id, $type, $categories = NULL, $brands = NULL)
    {
        ...
        ->or_where('products.type', 'production')
